  ...
  if (error && typeof error === 'object') {
    ...
    if (errorObj.status === 429) {
      return EvaluatorErrorHandler.createError(
        ...
        'AI service quota exceeded. The Gemini API free tier limit has been reached. Please try again later or contact the administrator.'
      );
    }
    
    // Check message for quota-related keywords
    if (typeof errorObj.message === 'string') {
      const msg = (errorObj.message as string).toLowerCase();
