/*
  # Complete AI Exam Evaluator Setup

  1. New Tables
    - `evaluations` - Store exam evaluation data
...
-- Clear existing admin data to avoid conflicts
DELETE FROM user_limits WHERE updated_by IN (SELECT id FROM admin_users);
DELETE FROM global_settings WHERE updated_by IN (SELECT id FROM admin_users);
DELETE FROM admin_users;

-- Insert default admin user
-- Password: Ece2131273642@ (hashed with SHA-256 + salt)
